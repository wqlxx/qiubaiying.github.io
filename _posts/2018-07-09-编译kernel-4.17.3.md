...
### 如何设定版本号  ### 	#### 版本号部分  ####
　　内核rpm包的命名规则是kernel-*base_num*.*base_sublevel*.*statble_update*决定的。
　　%define rpmversion 4.%{base_sublevel}.%{stable_update}
- base_sublevel：是spec文件中写死的
- statble_update: 这个字段是由稳定版本的补丁包序号决定，本次使用的patch-4.17.xz。故而该值为3。
#### release的显示值 ####
　　%define pkg_release %{fedora_build}%{?buildid}%{?dist}　　
- fedora_build
- 初始值为1	%global baserelease 1	%global fedora_build %{baserelease}
- buildid： 	\#define buildid .local
- dist： 　　保存在/usr/lib/rpm/macros.d/macros.dist。原生的fedora是这样的		[root@258f82502eea rpmbuild]# cat /usr/lib/rpm/macros.d/macros.dist		# dist macros.		%fedora                28		%dist                .fc28		%fc28                1  修改后  	[root@258f82502eea rpmbuild]# cat /usr/lib/rpm/macros.d/macros.dist		# dist macros.		%fedora                28		%dist                .WQLXX		%fc28                1　　baserelease： 修改为空  		%global baserelease %{nil}### prep阶段分析 ###
